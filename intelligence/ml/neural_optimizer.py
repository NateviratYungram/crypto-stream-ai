...
class AttentionLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Apply weights: [batch, seq, hidden] * [batch, seq, 1]
...
class NeuralV8Model(nn.Module):
    ...
    Added Batch Normalization and Dropout layers for robust convergence.
    """
    def __init__(self, input_size: int, hidden_size: int = 64, num_layers: int = 2):
        super(NeuralV8Model, self).__init__()
        self.hidden_size = hidden_size
        
        # BN handles feature scaling dynamically
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # BN expects [batch, features, seq] or [batch, features]
        b, s, f = x.shape
        x_flat = x.view(-1, f)
...
class SequentialTrainer:
    ...
    def _load_model(self):
        if os.path.exists(self.model_path):
            try:
                ...
                self.model.eval()
                print(f"NeuralV8: Loaded existing weights from {self.model_path}")
    ...
    def save_model(self):
        ...
        print(f"NeuralV8: Model saved to {self.model_path}")

    def predict(self, sequence: np.ndarray) -> Tuple[float, List[float]]:
        """
        Perform inference on a single sequence [window, features].
        Returns (WinProbability, AttentionWeights).
        """
        self.model.eval()
        with torch.no_grad():
            ...
            # weights shape: [1, seq, 1]
            attn_list = weights.squeeze().tolist()
            if isinstance(attn_list, float): attn_list = [attn_list]
            
        return float(prob.item()), attn_list

    def train_on_sequences(self, X: np.ndarray, y: np.ndarray, epochs: int = 150, lr: float = 0.001):
        """
        Train the V8 model with Validation Splitting and Early Stopping.
        """
        # Simple Validation Split (20%)
        split_idx = int(len(X) * 0.8)
        X_train, X_val = X[:split_idx], X[split_idx:]
        y_train, y_val = y[:split_idx], y[split_idx:]
        
        if len(X_val) == 0: # Fallback if dataset is too small
            X_train, y_train = X, y
            X_val, y_val = X, y
        ...
        # --- Intelligence V8: Asymmetric Clinical Precision Loss ---
        # Penalizes False Positives (losing trades) more than False Negatives
        def risk_aware_bce(output, target, fp_weight=3.0):
            ...
            # Risk-Aware:  -[y*log(p) + fp_weight * (1-y)*log(1-p)]
            loss = target * torch.log(output) + fp_weight * (1.0 - target) * torch.log(1.0 - output)
            return torch.neg(torch.mean(loss))

        print(f"NeuralV8: Training on {len(X_train)} samples, validating on {len(X_val)} (Risk-Aware Active)...")
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 10 == 0:
                print(f"Epoch [{epoch+1}/{epochs}], Train Loss: {batch_loss.item():.4f}, Val Loss: {val_loss.item():.4f}")
            ...
            if val_loss < best_val_loss:
                ...
                if trigger_times >= patience:
                    print(f"NeuralV8: Early stopping at epoch {epoch+1}")
                    break
